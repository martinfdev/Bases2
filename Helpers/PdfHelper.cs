...
namespace AppPdfGenAccountStatus.Helpers
{
	public class PdfHelper
	{
		...
		{
			var listTrx = vtcObtenerMovimientosCuentaMes?.VtcObtenerMovimientosCuentaMesResult?.Detalle?? [];
			...
			for (int i = 0; i < listTrx.Length; i++)
			{
				...
				if (listTrx[i].CodTra == "5")
				{
					gfx.DrawString(listTrx[i]?.FechaProceso?? "No Data", font, XBrushes.Black, new XRect(startX + 11.0, currentY + 5.0, 30.0, 10.0), XStringFormats.TopLeft);
					gfx.DrawString(listTrx[i]?.FechaOrigen?? "No data", font, XBrushes.Black, new XRect(startX + 70.0, currentY + 5.0, 30.0, 10.0), XStringFormats.TopLeft);
					gfx.DrawString(listTrx[i]?.Descripcion?? "No Data", font, XBrushes.Black, new XRect(startX + 130.0, currentY + 5.0, 30.0, 10.0), XStringFormats.TopLeft);
					if (listTrx[i].Moneda.Equals("QUETZAL"))
					{
